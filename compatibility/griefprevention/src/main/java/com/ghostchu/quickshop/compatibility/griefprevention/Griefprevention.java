/*
 *  This file is a part of project QuickShop, the name is Griefprevention.java
...
public final class Griefprevention extends JavaPlugin implements Listener {
    ...
    public void onEnable() {
        ...
        getLogger().info("QuickShop Compatibility Module - GriefPrevention loaded");
    }
    ...
    public void onSubClaimCreated(ClaimCreatedEvent event) {
        ...
        for (Chunk chunk : event.getClaim().getChunks()) {
            Map<Location, Shop> shops = this.api.getShopManager().getShops(chunk);
            if (shops != null) {
                for (Shop shop : shops.values()) {
                    if (!shop.getOwner().equals(event.getClaim().getOwnerID()) &&
                            event.getClaim().contains(shop.getLocation(), false, false)) {
                        this.api.logEvent(new ShopRemoveLog(Util.getSenderUniqueId(event.getCreator()), String.format("[%s Integration]Shop %s deleted caused by [Single] SubClaim Created", this.getName(), shop), shop.saveToInfoStorage()));
                        shop.delete();
                    }
                }
            }
        ...
        for (Chunk chunk : claim.getChunks()) {
            ...
            for (Shop shop : shops.values()) {
                ...
                if (event.getIdentifier().equals(shop.getOwner().toString())) {
                    this.api.logEvent(new ShopRemoveLog(Util.getSenderUniqueId(event.getChanger()), String.format("[%s Integration]Shop %s deleted caused by [Single] Claim/SubClaim Trust Changed", this.getName(), shop), shop.saveToInfoStorage()));
                    shop.delete();
                } else if (event.getIdentifier().contains(shop.getOwner().toString())) {
                    this.api.logEvent(new ShopRemoveLog(Util.getSenderUniqueId(event.getChanger()), String.format("[%s Integration]Shop %s deleted caused by [Group] Claim/SubClaim Trust Changed", this.getName(), shop), shop.saveToInfoStorage()));
                    shop.delete();
                } else if ("all".equals(event.getIdentifier()) || "public".equals(event.getIdentifier())) {
                    this.api.logEvent(new ShopRemoveLog(Util.getSenderUniqueId(event.getChanger()), String.format("[%s Integration]Shop %s deleted caused by [All/Public] Claim/SubClaim Trust Changed", this.getName(), shop), shop.saveToInfoStorage()));
                    shop.delete();
                }
            }
        }
            ...
            if (shops != null) {
                for (Shop shop : shops.values()) {
                    if (claim.contains(shop.getLocation(), false, false)) {
                        this.api.logEvent(new ShopRemoveLog(Util.getNilUniqueId(), String.format("[%s Integration]Shop %s deleted caused by [System] Claim/SubClaim Unclaimed/Expired: " + logMessage, this.getName(), shop), shop.saveToInfoStorage()));
                        shop.delete();
                    }
                }
            }
            ...
            if (shops != null) {
                for (Shop shop : shops.values()) {
                    if (!shop.getOwner().equals(subClaim.getOwnerID()) &&
                        ...
                        this.api.logEvent(new ShopRemoveLog(Util.getNilUniqueId(), String.format("[%s Integration]Shop %s deleted caused by [Single] SubClaim Unclaimed", this.getName(), shop), shop.saveToInfoStorage()));
                        shop.delete();
                    }
                }
            }
            ...
            if (shops != null) {
                for (Shop shop : shops.values()) {
                    if (oldClaim.contains(shop.getLocation(), false, false) &&
                        ...
                        this.api.logEvent(new ShopRemoveLog(Util.getNilUniqueId(), String.format("[%s Integration]Shop %s deleted caused by [Single] Claim Resized: ", this.getName(), shop), shop.saveToInfoStorage()));
                        shop.delete();
                    }
                }
            }
            ...
            if (shops != null) {
                for (Shop shop : shops.values()) {
                    if (!shop.getOwner().equals(claimVerifyChunks.getOwnerID()) &&
                        ...
                        this.api.logEvent(new ShopRemoveLog(Util.getNilUniqueId(), String.format("[%s Integration]Shop %s deleted caused by [Single] SubClaim Resized: ", this.getName(), shop), shop.saveToInfoStorage()));
                        shop.delete();
                    }
                }
            }
