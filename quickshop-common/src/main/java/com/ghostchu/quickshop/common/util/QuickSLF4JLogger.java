...
public class QuickSLF4JLogger {
  ...
  /**
   * This creates a SLF4J logger. In the process it initializes the SLF4J service provider. This
   * method looks for the provider in the plugin jar instead of in the server jar when creating a
   * Logger. The provider is only initialized once, so this method should be called early.
   * <p>
   * The provider is bound to the service class `SLF4JServiceProvider`. Relocating this class makes
   * it available for exclusive own usage. Other dependencies will use the relocated service too,
   * and therefore will find the initialized provider.
   *
  ...
  public static Logger initializeLoggerService(final java.util.logging.Logger parent) {
    ...
    } catch(final Throwable reflectEx) {
