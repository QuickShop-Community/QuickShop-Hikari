 ...
 * @since 6.2.0.9
 */
public class SetAmountComponent implements ControlComponent {
  ...
  public boolean applies(final @NotNull QuickShopAPI plugin, final @NotNull Player sender, final @NotNull Shop shop) {
    ...
    if(((QuickShop)plugin).isAllowStack()) {

      return ((QuickShop)plugin).perm().hasPermission(sender, "quickshop.other.amount") ||
             shop.playerAuthorize(sender.getUniqueId(), BuiltInShopPermission.SET_STACK_AMOUNT) &&
             ((QuickShop)plugin).perm().hasPermission(sender, "quickshop.create.changeamount");
    }
    return false;
  }
  ...
  public Component generate(final @NotNull QuickShopAPI plugin, final @NotNull Player sender, final @NotNull Shop shop) {
    ...
    final Component text = ((QuickShop)plugin).text().of(sender, "controlpanel.stack", shop.getItem().getAmount()).forLocale();
    final Component hoverText = ((QuickShop)plugin).text().of(sender, "controlpanel.stack-hover").forLocale();
