 ...
 * @since 6.2.0.11
 */
public class InventoryComponent implements ControlComponent {
  ...
  public boolean applies(final @NotNull QuickShopAPI plugin, final @NotNull Player sender, final @NotNull Shop shop) {
    ...
    return shop.playerAuthorize(sender.getUniqueId(), BuiltInShopPermission.ACCESS_INVENTORY)
           || QuickShop.getInstance().perm().hasPermission(sender, "quickshop.other.open");
  }
  ...
  public Component generate(final @NotNull QuickShopAPI plugin, final @NotNull Player sender, final @NotNull Shop shop) {
    ...
    final Component text = ((QuickShop)plugin).text().of(sender, "controlpanel.inventory").forLocale();
    final Component hoverText = ((QuickShop)plugin).text().of(sender, "controlpanel.inventory-hover").forLocale();
