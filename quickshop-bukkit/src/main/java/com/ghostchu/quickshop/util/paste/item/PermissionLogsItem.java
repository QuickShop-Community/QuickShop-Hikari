...
public class PermissionLogsItem implements SubPasteItem {
    ...
    private String buildContent() {
        ...
        List<String> debugLogs = Log.fetchLogs(Log.Type.PERMISSION).stream().map(recordEntry -> "[" + format.format(recordEntry.getTimestamp()) + "] " + recordEntry.syncBuildString()).toList();
